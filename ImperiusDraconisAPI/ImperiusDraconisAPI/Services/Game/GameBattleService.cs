...
public sealed class GameBattleService
{
    ...
    public async Task<AutomaticBattleResponse> BattleAsync(AutomaticBattleRequest request,string idempotencyKey,CancellationToken ct)
    {
        ...
        try
        {
            ...
            {
                var playerDamage=Math.Max(1,player.Attack-opponent.Defense/2+Random.Shared.Next(-2,3));
                var opponentDamage=Math.Max(1,opponent.Attack-player.Defense/2+Random.Shared.Next(-2,3));
    ...
    private static async Task<Fighter> ReadPlayerAsync(MySqlConnection c,MySqlTransaction t,long roblox,CancellationToken ct)
    {
        await using var cmd=new MySqlCommand(
            """SELECT D.Id,D.IdAlumno,D.Name,D.SpeciesCode,D.Rarity,D.Level,D.Life,D.Status,DD.BaseAttack,DD.BaseDefense,COALESCE(A.Dracoins,0) FROM GameRobloxLinks L INNER JOIN Alumnos A ON A.IdAlumno=L.IdAlumno INNER JOIN GameDragons D ON D.IdAlumno=A.IdAlumno AND D.Selected=1 INNER JOIN GameDragonDefinitions DD ON DD.Code=D.SpeciesCode WHERE L.RobloxUserId=@Roblox AND L.Active=1 AND A.Activo=1 FOR UPDATE;""",c,t);
        ...
        return new Fighter(r.GetInt64(0),r.GetInt32(1),null,r.GetString(2),r.GetString(3),r.GetString(4),r.GetInt32(5),r.GetInt32(6),r.GetString(7),Scale(r.GetInt32(8),r.GetInt32(5)),Scale(r.GetInt32(9),r.GetInt32(5)),r.GetDecimal(10));
    }
    private static async Task<Fighter?> ReadOpponentAsync(MySqlConnection c,MySqlTransaction t,Fighter p,CancellationToken ct)
    {
        await using var cmd=new MySqlCommand(
            """SELECT D.Id,D.IdAlumno,D.Name,D.SpeciesCode,D.Rarity,D.Level,D.Life,D.Status,DD.BaseAttack,DD.BaseDefense FROM GameDragons D INNER JOIN GameDragonDefinitions DD ON DD.Code=D.SpeciesCode WHERE D.Selected=1 AND D.Status='ACTIVE' AND D.IdAlumno<>@IdAlumno ORDER BY ABS(D.Level-@Level),D.Id LIMIT 1;""",c,t);
        ...
        return new Fighter(r.GetInt64(0),r.GetInt32(1),null,r.GetString(2),r.GetString(3),r.GetString(4),r.GetInt32(5),r.GetInt32(6),r.GetString(7),Scale(r.GetInt32(8),r.GetInt32(5)),Scale(r.GetInt32(9),r.GetInt32(5)),0);
    }
    private static async Task<Fighter> ReadWildAsync(MySqlConnection c,MySqlTransaction t,int level,CancellationToken ct)
    {
        await using var cmd=new MySqlCommand("SELECT Code,DisplayName,SpeciesCode,Rarity,LEAST(MaxLevel,GREATEST(MinLevel,@Level)),BaseLife,BaseAttack,BaseDefense FROM GameWildDragonDefinitions WHERE Active=1 AND @Level BETWEEN MinLevel AND MaxLevel ORDER BY MinLevel DESC LIMIT 1;",c,t);cmd.Parameters.AddWithValue("@Level",level);
        ...
        return new Fighter(null,null,r.GetString(0),r.GetString(1),r.GetString(2),r.GetString(3),opponentLevel,r.GetInt32(5),"ACTIVE",Scale(r.GetInt32(6),opponentLevel),Scale(r.GetInt32(7),opponentLevel),0);
    }
    ...
    private sealed record Fighter(long? Id,int? IdAlumno,string? WildCode,string Name,string SpeciesCode,string Rarity,int Level,int Life,string Status,int Attack,int Defense,decimal Balance);
}
